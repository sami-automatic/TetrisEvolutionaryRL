...
class GeneticEngineGenerator(object):
    engines = []
    rewards = []
    ...
    def generate_random_engines():
        for _ in range(NUMBER_OF_ENGINES):
            parameters = {
                clear_line_reward: np.random.uniform(0.1, 30.0)     # 15.0
                height_penalty: np.random.uniform(-0.1, -1.0)       # -0.810066
                hole_penalty: np.random.uniform(-0.1, -0.5)         # -0.36
                bumpiness_penalty: np.random.uniform(-0.1, -0.5)    # -0.18
                game_over_penalty: np.random.uniform(-0.1, -0.5)    # -0.86
            }
    ...
    def eliminate_engines(self):
        ...
        reward_engine_tuple = sorted(reward_engine_tuple, key=lambda x: x[0], reverse=True)
        engines = [reward_engine_tuple[i][1] for i in range(NUMBER_OF_ENGINES // 2 + 1)]
